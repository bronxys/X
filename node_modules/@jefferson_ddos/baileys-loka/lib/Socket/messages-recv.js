        ...
        try {
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    // if (node?.attrs?.addressing_mode === 'lid' && (jidNormalizedUser(node?.attrs?.participant) === jidNormalizedUser(authState.creds.me!.lid))) {
                    // 	// @ts-ignore
                    // 	msg.key.jid = jidNormalizedUser(authState.creds.me!.id)
                    // }
                    // if (node?.attrs?.addressing_mode === 'lid' && node?.attrs?.participant_pn) {
                    // 	// @ts-ignore
                    // 	msg.key.jid = jidNormalizedUser(node?.attrs?.participant_pn)
                    // }
