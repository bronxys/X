...
class ReaddirGlob extends events_1.EventEmitter {
    constructor(cwd, options, cb) {
        ...
        if (this.options.pattern) {
            ...
            this.matchers = matchers.map(m => new minimatch_1.Minimatch(m, {
        ...
        if (this.options.ignore) {
            ...
            this.ignoreMatchers = ignorePatterns.map(ignore => new minimatch_1.Minimatch(ignore, { dot: true }));
        }
        this.skipMatchers = [];
        if (this.options.skip) {
            const skipPatterns = Array.isArray(this.options.skip) ? this.options.skip : [this.options.skip];
            this.skipMatchers = skipPatterns.map(skip => new minimatch_1.Minimatch(skip, { dot: true }));
        }
        this.iterator = explore((0, path_1.resolve)(cwd || '.'), this.options.follow, this.options.stat, this._shouldSkipDirectory.bind(this));
        this.paused = false;
        this.inactive = false;
        this.aborted = false;
        if (cb) {
...
exports.ReaddirGlob = ReaddirGlob;
const readdirGlob = (pattern, options, cb) => new ReaddirGlob(pattern, options, cb);
exports.readdirGlob = readdirGlob;
exports.readdirGlob.ReaddirGlob = ReaddirGlob;
